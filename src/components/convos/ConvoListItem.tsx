...
const useRelativeTime = (lastUpdatedAtNs: bigint | undefined) => {
  ...
  const nsRef = useRef(lastUpdatedAtNs);
  nsRef.current = lastUpdatedAtNs;
  const interval = useInterval(() => {
    setTime(formatTime(nsRef.current));
  }, 60_000);
  ...
  useEffect(() => {
    ...
    // interval is stable from useInterval
...
export const ConvoListItem: React.FC<ConvoListItemProps> = memo(
  ({ convo, selected }) => {
